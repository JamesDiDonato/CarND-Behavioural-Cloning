...
[image13]: ./ReportPics/BeforeCrop.png "Before Cropping"
[image14]: ./ReportPics/AfterCrop.png "After Cropping"



## Rubric Points
### Here I will consider the [rubric points](https://review.udacity.com/#!/rubrics/432/view) individually and describe how I addressed each point in my implementation. The code repository can be found at [github link](https://github.com/JamesDiDonato/CarND-P3).
...
Secondly, the model contains a dropout layer with probability 0.5 in order to reduce overfitting (model.py lines 104). The architecture contains a significant number of parameters and therefore is subject to overfitting. Therefore before inputs are fed to the fully connected layer, they are dropped out to help prevent this phenomena.


#### 3. Model parameter tuning

The model used an adam optimizer, so the learning rate was not tuned manually (model.py line 114), however the number of epochs and batch_size were. For starters, the number of EPOCHS was calibrated to a value of 5 after training the chosen model for EPOCHS = 3,10, and anylzing the results. The images below plot the MSE loss for both training and validation: 
...
In comparison to LeNet, this architecture peformed very similar in terms of MSE loss values in the ~0.03 range after 5 EPOCHS of training:
...
At this point, I decided to augment the data set by adding the left and right images from te side camera with a steering offset of 0.2. The steering offset was required to correct against the fact that the cameras were not pointed straight and represnted an angle offset from the steering wheel. Now the total data set size was 21607, which yielded training and validation set sizes of 17286 and 4321 respectively.
